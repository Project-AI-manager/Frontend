...
export default function ChannelsPage() {
  ...
  async function submitPersonalAccount(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (authStep === "phone") {
        const response = await telegramApi.startAccount({
          phone: phone.trim(),
        });

        setAuthChannelId(response.channel_id);
        ...
        if (response.status === "password_required") {
          ...
          await completePersonalAccount(response.display_name);
        }
      } else if (authStep === "password" && authChannelId) {
        const response = await telegramApi.confirmPassword({
          channel_id: authChannelId,
          password,
        });
        setPassword("");
